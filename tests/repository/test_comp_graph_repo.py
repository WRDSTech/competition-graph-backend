...
def test_get_surrounding_node_by_center(comp_graph_repo):
    ...
    assert EntityRelation(id="1", category="product", source="6", target="5") in partial_graph.links
    assert EntityRelation(id="1", category="product", source="5", target="6") not in partial_graph.links
